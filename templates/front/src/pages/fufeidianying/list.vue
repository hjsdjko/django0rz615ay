...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"172px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"right","justifyContent":"space-between","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"380px","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>电影名称：</div>
	  ...
	  <el-button v-if="btnAuth('fufeidianying','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"100px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/fufeidianyingAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			<img @click.stop="imgPreView(item.dianyingfengmian)" v-if="item.dianyingfengmian && item.dianyingfengmian.substr(0,4)=='http'" :src="item.dianyingfengmian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.dianyingfengmian?item.dianyingfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.dianyingfengmian?item.dianyingfengmian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"30% 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"block","textIndent":"2em","overflow":"hidden","borderWidth":"0 1px 0 0","width":"48%","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"100%"}'>{{item.dianyingjieshao}}</div>
		      <div class="info" :style='{"padding":"30% 0 0 0px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","display":"block","width":"48%","height":"100%"}'>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">电影名称:{{item.dianyingmingcheng}}</div>
	...
	<!-- 热门信息 -->
	<div class="hot" :style='{"padding":"10px 4px","margin":"20px 0 0","borderRadius":"12px","background":"#fff","width":"380px","float":"right","height":"auto","order":"5"}'>
	  ...
	  <div :style='{"padding":"30px 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"100%","margin":"0 0 20px","background":"#fff","height":"120px"}' @click="toDetail(item)">
	      <img :style='{"width":"48%","objectFit":"cover","float":"left","display":"block","height":"100%"}' v-if="item.dianyingfengmian && item.dianyingfengmian.substr(0,4)=='http'" :src="item.dianyingfengmian.split(',')[0]" alt="">
	      <img :style='{"width":"48%","objectFit":"cover","float":"left","display":"block","height":"100%"}' v-else :src="baseUrl + (item.dianyingfengmian?item.dianyingfengmian.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#333","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0px","width":"48%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.dianyingmingcheng}}</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/dianyingleixing/dianyingleixing').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({name:'电影类型',list:this.dianyingleixingOptions,tableName: 'dianyingleixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getHotList() {
        let autoSortUrl = "";
        autoSortUrl = "fufeidianying/autoSort";
      ...
      getFenlei() {
		this.$http.get('dianyingleixing/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'dianyingleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`fufeidianying/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/fufeidianyingDetail', query: params});
      },
