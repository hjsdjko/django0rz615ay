...
<div class="home-preview" :style='{"margin":"0px auto","flexWrap":"wrap","flexDirection":"row","background":"#fff","display":"flex","width":"100%","justifyContent":"center"}'>
	...
	<div id="animate_newsnews" class="news animate__animated" :style='{"padding":"60px 0 40px","margin":"0 auto","background":"url(),#bbc1c4","flex":"1","width":"100%","position":"relative","order":"5"}'>
		...
		<div class="title" :style='{"width":"86%","margin":"0px auto","lineHeight":"60px","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231010/84c0d484cc5144b0b970462a64801e94.png) no-repeat 520px center"}'>
			<span :style='{"color":"#fff","fontSize":"50px","fontWeight":"600"}'>电影资讯</span>
		</div>
...
<div id="animate_recommendmianfeidianying" class="recommend animate__animated" :style='{"padding":"60px 7% 20px","margin":"0 auto","borderColor":"#ddd","background":"url(http://codegen.caihongy.cn/20231010/dab28806e6084effb4866358e938b0b4.jpg) no-repeat center top / 100% 100%","borderWidth":"0px 0","width":"100%","position":"relative","borderStyle":"solid","order":"2"}'>
    ...
    <div class="title" :style='{"width":"100%","margin":"0px auto","lineHeight":"60px","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231010/84c0d484cc5144b0b970462a64801e94.png) no-repeat 520px center"}'>
		<span :style='{"padding":"0","fontSize":"50px","color":"#fff","fontWeight":"600","display":"block"}'>免费电影推荐</span>
	</div>
	...
	<div class="list list3 index-pv1">
		<div :style='{"width":"100%","padding":"10px 10px 0","margin":"60px 0","background":"#fff","height":"auto"}' class="swiper-container" id="recommendmianfeidianying">
			<div class="swiper-wrapper">
				<div class="swiper-slide animation-box" :style='{"border":"0","cursor":"pointer","padding":"0px 0px 20px","boxShadow":"1px 2px 4px #ccc,inset -8px 0px 6px 0px #e6e6e6","borderRadius":"0px","background":"#fff","position":"relative"}' v-for="(item,index) in mianfeidianyingRecommend" :key="index" @click="toDetail('mianfeidianyingDetail', item)">
					<img :name="item.id" :style='{"objectFit":"cover","width":"100%","height":"480px"}' v-if="preHttp(item.dianyingfengmian)" :src="item.dianyingfengmian.split(',')[0]" alt="" />
					<img :name="item.id" :style='{"objectFit":"cover","width":"100%","height":"480px"}' v-else :src="baseUrl + (item.dianyingfengmian?item.dianyingfengmian.split(',')[0]:'')" alt="" />
					<div class="line1" :style='{"padding":"0 10px","margin":"6px 0 0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","background":"none","width":"calc(100% - 104px)","fontSize":"14px","lineHeight":"32px","textOverflow":"ellipsis","fontWeight":"600","height":"32px"}'>电影名称:{{item.dianyingmingcheng}}</div>
	...
	<div @click="moreBtn('mianfeidianying')" :style='{"border":"0px solid #fff","cursor":"pointer","margin":"12px auto","top":"60px","textAlign":"center","left":"calc(7% + 400px)","background":"none","display":"block","width":"auto","lineHeight":"36px","position":"absolute"}'>
		<span :style='{"color":"#fff","fontSize":"16px","textDecoration":"underline"}'>查看更多</span>
...
<!-- 特价商品 -->
<div id="animate_listfufeidianying" class="lists animate__animated" :style='{"width":"100%","padding":"20px 0 0","margin":"40px auto 20px","position":"relative","background":"#fff","order":"5"}'>
	...
	<div class="title" :style='{"width":"86%","margin":"0px auto","lineHeight":"60px","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231006/53e70b77fda2495f9e803a7eca8d56c1.png) no-repeat 520px center"}'>
	  <span :style='{"padding":"0","fontSize":"50px","color":"#1e3c4f","fontWeight":"600","display":"block","font":"\"宋体\""}'>付费电影展示</span>
	</div>
	...
	<div class="list list2 index-pv1" :style='{"padding":"0 7%","margin":"40px 0 0","color":"#999","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between","height":"auto"}'>
		<div :style='{"cursor":"pointer","boxShadow":"1px 2px 9px #ccc","padding":"10px","margin":"0 0 30px","background":"none","display":"flex","width":"49%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item,index) in fufeidianyingList" class="list-item animation-box" :key="index" @click="toDetail('fufeidianyingDetail', item)">
			<img :style='{"width":"40%","objectFit":"cover","display":"inline-block","height":"400px"}' v-if="preHttp(item.dianyingfengmian)" :src="item.dianyingfengmian.split(',')[0]" alt="" />
			<img :style='{"width":"40%","objectFit":"cover","display":"inline-block","height":"400px"}' v-else :src="baseUrl +  (item.dianyingfengmian?item.dianyingfengmian.split(',')[0]:'')" alt="" />
			<div :style='{"width":"60%","padding":"100px 60px","overflow":"hidden","background":"none","display":"inline-block","height":"400px"}' class="item-info">
				<div class="name line1" :style='{"padding":"0 0px","margin":"0 0 20px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"32px","fontSize":"16px","textOverflow":"ellipsis"}'>电影名称:{{item.dianyingmingcheng}}</div>
				<div :style='{"float":"right","padding":"0 10px","display":"inline-block"}'>
	...
	<div @click="moreBtn('fufeidianying')" :style='{"border":"0px solid #1d8001","cursor":"pointer","margin":"32px auto","top":"0","textAlign":"center","left":"calc(7% + 400px)","background":"#fff","display":"block","width":"auto","lineHeight":"36px","textDecoration":"underline","position":"absolute"}'>
    ...
    methods: {
		...
		handleScroll() {
			let arr = [
				...
				{id:'animate_recommendmianfeidianying',css:'animate__'},
				{id:'animate_listfufeidianying',css:'animate__'},
      ...
      getSystemIntroduction() {
		...
		getList() {
			let autoSortUrl = "";
			let data = {}
          autoSortUrl = "mianfeidianying/autoSort";
          if(localStorage.getItem('frontToken')) {
              autoSortUrl = "mianfeidianying/autoSort2";
          }
			data = {
				page: 1,
				limit: 8,
			...
			this.$http.get(autoSortUrl, {params: data}).then(res => {
				if (res.data.code == 0) {
					this.mianfeidianyingRecommend = res.data.data.list;
					...
					this.$nextTick(() => {
						new Swiper('#recommendmianfeidianying', options)
					})
			...
			this.$http.get('fufeidianying/list', {params: data}).then(res => {
				if (res.data.code == 0) {
					this.fufeidianyingList = res.data.data.list;
