<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					...
					<el-button class="btn18" v-if="isAuth('dianyingxinxi','名称')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						名称
					</el-button>
					<el-button class="btn18" v-if="isAuth('dianyingxinxi','评分统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						评分统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('dianyingxinxi','类型分析')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						类型分析
					</el-button>
					<el-button class="btn18" v-if="isAuth('dianyingxinxi','地区统计')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						地区统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('dianyingxinxi','演员分析')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						演员分析
					</el-button>
				</el-row>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="movienameChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="leixingChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="diquChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="zhuyanChart5" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var movienameChart1 = echarts.init(document.getElementById("movienameChart1"),'macarons');
        this.$http({
            url: "dianyingxinxi/group/moviename",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                var option = {};
                // 使用刚指定的配置项和数据显示图表。
                movienameChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    movienameChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var leixingChart3 = echarts.init(document.getElementById("leixingChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                leixingChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    leixingChart3.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog4() {
      ...
      this.$nextTick(()=>{
        ...
        var diquChart4 = echarts.init(document.getElementById("diquChart4"),'macarons');
        this.$http({
...
//统计接口
    chartDialog5() {
      ...
      this.$nextTick(()=>{
        ...
        var zhuyanChart5 = echarts.init(document.getElementById("zhuyanChart5"),'macarons');
        this.$http({
